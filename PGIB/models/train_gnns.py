...
def train_GC(model_type):
	...
	# attacker = RandomAttack(data)
	    #attacker.reset()
	    #attacker.attack(num_budgets=0.1) # attacking the graph with 10% edges perturbations
	    #attacked_data = attacker.data()
	    #tmp.append(attacked_data)
    #dataset = tmp
    ...
    for epoch in range(train_args.max_epochs):
        ...
        append_record("Epoch {:2d}, loss: {:.3f}, acc: {:.5f}".format(epoch, np.average(loss_list), np.concatenate(acc, axis=0).mean()))
        ...
        append_record("Eval epoch {:2d}, loss: {:.3f}, acc: {:.5f}".format(epoch, eval_state['loss'], eval_state['acc']))

        test_state, _, _ = test_GC(dataloader['test'], gnnNets, criterion)
        print(f"Test Epoch: {epoch} | Loss: {test_state['loss']:.5f} | Acc: {test_state['acc']:.5f}")           
    ...
    gnnNets.to_device()
    test_state, _, _ = test_GC(dataloader['test'], gnnNets, criterion)
    print(f"Test | Dataset: {data_args.dataset_name:s} | model: {model_args.model_name:s}_{model_type:s} | Loss: {test_state['loss']:.3f} | Acc: {test_state['acc']:.5f}")
    append_record("loss: {:.3f}, acc: {:.5f}".format(test_state['loss'], test_state['acc']))
